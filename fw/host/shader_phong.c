...
Float4 vshader_phong (Object *obj, size_t face_idx, size_t vtx_idx, Varying *vry, const int vshader_idx, gpu_cfg_t *cfg) {
	
	// transform 3d coords of the vertex to homogenous clip coords
	Float3 model   = wfobj_get_vtx_coords (obj->wfobj, face_idx, vtx_idx, vshader_idx);
	...
	Float3 norm       = wfobj_get_norm_coords    (obj->wfobj, face_idx, vtx_idx, vshader_idx);
	...
	if (obj->texture != NULL) {
		Float2 texture = wfobj_get_texture_coords (obj->wfobj, face_idx, vtx_idx, vshader_idx);
